...
public class TargetOptionDisplay : MonoBehaviour
{
    ...
    public void setBackgroundSelected(bool selected)
    {
        if (selected)
        {
            GetComponent<Image>().color = new Color(onHighlightColor.r, onHighlightColor.g, onHighlightColor.b, onHighlightColor.a);
        }
        else
        {
            GetComponent<Image>().color = new Color(defaultColor.r, defaultColor.g, defaultColor.b, defaultColor.a);
        }
        // Debug.Log("I was set to background "+selected );
    }
